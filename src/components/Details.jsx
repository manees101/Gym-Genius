...
const Details = ({exerciseDetails}) => {
    const {name,target,gifUrl,instructions,equipment,bodyPart}=exerciseDetails;
    const exerciseObj=[
        {
            icon:BodyPartImg,
            name:bodyPart
  ...
  return (
    <Stack gap="30px">
    <Stack direction={{lg:"row"}} p={20} alignItems="center" justifyContent="space-around" gap="60px">
    <img src={gifUrl} alt={name} style={{width:"400px",height:"500px"}}/>
    <Stack gap={{lg:"35px",xs:"20x"}}>
        ...
        <Typography>
            ...
            <br/>one of the best exercises for your {target}.
    ...
    {instructions?.map((item,index)=><Typography variant='h6'>
       {index+1}. {item}
    </Typography>)}
    </Stack>
